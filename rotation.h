...
#include <iostream>
template<typename T>
Eigen::Matrix4d GetHomogenousMatrix(T camera){
    cv::Vec3d r (camera[0],camera[1],camera[2]);
    cv::Mat R;
    cv::Rodrigues(r ,  R );
    
    Eigen::Matrix3d eigenRotationMatrix;
    cv::cv2eigen(R, eigenRotationMatrix);


    Eigen::Vector3d t (camera[3], camera[4], camera[5]);
    Eigen::RowVector4f bottomArray (0,0,0,1);

    std::cout << bottomArray.transpose();

    // Eigen::Matrix<T,3,4> a = Eigen::ColStack<Eigen::Matrix3d,Eigen::Matrix3d>(eigenRotationMatrix,t);
    // Eigen::Matrix<T,4,4> H = Eigen::RowStack<Eigen::Matrix,Eigen::Matrix4d>(a,bottomArray);
    Eigen::Matrix4d H (4,4);
    H << eigenRotationMatrix(0,0),eigenRotationMatrix(0,1),eigenRotationMatrix(0,2),camera[3],
        eigenRotationMatrix(0,1),eigenRotationMatrix(1,1),eigenRotationMatrix(1,2),camera[4],
        eigenRotationMatrix(0,2),eigenRotationMatrix(1,2),eigenRotationMatrix(2,2),camera[5],
        0,0,0,1;
    return H;
}
